...
and the nth number is the sum of the (n - 1)th and (n - 2)th numbers. Write a function that takes in an integer `n` and returns the nth Fibonacci number.
...
export function getNthFibRecursive(n: number): number {
  if (n === 2) return 1
  if (n === 1) return 0
