...
The function should use a variation of the Binary Search algorithm to determine if the target integer is contained in the array and 
should return its index if it is, otherwise -1.

If you're unfamiliar with Binary Search, we recommend watching the Conceptual Overview section of the Binary Search question's video explanation 
...
export function shiftedBinarySearch(array: number[], target: number) {
  ...
  while (left <= right) {
    ...
    if (middleNum === target) {
      ...
      if (middleNum < target && target <= rightNum) {
        left = middle + 1
      } else {
        right = middle - 1
      }
