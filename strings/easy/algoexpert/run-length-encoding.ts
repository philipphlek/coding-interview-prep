...
export function runLengthEncoding(string: string) {
  ...
  for (let i = 1; i < string.length; i++) {
    ...
    if (prevLetter !== currLetter || runningLength === 9) {
      newString.push(runningLength.toString())
      newString.push(prevLetter)
