...
// O(n) time - n is number of competitions | O(k) space - k is number of teams
export function tournamentWinner(competitions: string[][], results: number[]) {
  let currBestTeam = ''
  let pointTracker: Record<string, number> = {
    [currBestTeam]: 0,
  }
  for (let i = 0; i < competitions.length; i++) {
    const [homeTeam, awayTeam] = competitions[i]
    const isWinnerHomeTeam = results[i] === 1
    const winningTeam = isWinnerHomeTeam ? homeTeam : awayTeam
    if (pointTracker[winningTeam]) {
      pointTracker[winningTeam]++
    } else {
      pointTracker[winningTeam] = 1
    }
    if (pointTracker[winningTeam] > pointTracker[currBestTeam]) {
