...
export function bestSeat(seats: number[]) {
  const bestSeat: BestSeat = {
    index: -1,
    consecutiveEmpty: 0,
  }
  for (let i = 1; i < seats.length - 1; i++) {
    let consecutiveEmpty = 0
    while (seats[i + consecutiveEmpty] === 0) {
      consecutiveEmpty++
    }
    if (consecutiveEmpty > bestSeat.consecutiveEmpty) {
      bestSeat.index = Math.floor((consecutiveEmpty - 1) / 2) + i
      bestSeat.consecutiveEmpty = consecutiveEmpty
    }
    i += consecutiveEmpty
  }
  return bestSeat.index
}
